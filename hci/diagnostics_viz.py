...
def viz_infer_geometry(
    ...
    *,
    ...
    rows = [
        (
            rho_coll,
            r"row 1 — $\rho_{\mathrm{coll}}$ (collinear facilitation)",
            r"$\rho_{\mathrm{coll}}$",
        ),
        (
            surround,
            r"row 2 — $S$ (cross-orientation surround pool; divisive term)",
            r"$S$",
        ),
    ]
    ...
    fig.suptitle(
        r"Seed geometry — collinear readback $\rho_{\mathrm{coll}}$ and cross surround $S$",
        fontsize=10,
        color=VIZ.FG,
        fontfamily="monospace",
    )
