r"""infer.py — HCI single-image inference"""
...
def _n_interior(is_border_grid) -> int:
    border = np.asarray(is_border_grid, dtype=bool)
...
def _format_run_settings(
    ...
    *,
    ...
    return [
        "Run",
        f"  device={device}  diagnostics={diagnostics}  "
        f"ridge_nms={int(ridge_nms)}  threshold={threshold:.2f}  "
        f"n_interior={n_interior}",
    ]


def _format_model_summary(model) -> list[str]:
    lines = [
        "Model",
        f"  {format_model_param_summary(model)} parameters",
        "",
        "L0",
        ...
        *format_seed_param_lines(model.seed),
        "",
        "Renderer",
        *format_renderer_param_lines(model.renderer),
    ]
    return lines
...
def main():
    ...
    if args.verbose:
        for line in _format_model_summary(model):
            print(line)
        print()
    ...
    if args.verbose:
        for line in _format_run_settings(
            device, args.diagnostics,
            ridge_nms=args.ridge_nms, threshold=float(args.threshold),
            n_interior=_n_interior(prep["is_border_grid"]),
        ):
            print(line)
        print()

    collect_diags = args.diagnostics
    (
        bmap,
        theta_map,
        rho_post,
