...
2. **L1** — $\cos^2$ hypercolumns on $h_{2m}$; **no seed NR** — recurrence starts from raw $\boldsymbol{\mu}$; **collinear passes** use raw-space $\beta_{\cdot}$, nonnegative $u$, then **one** divisive NR per pass with **spatial** $\eta^{(t)}(c)=\eta_0\cdot\sigma(\mathrm{MLP}(\bar\kappa,\bar z))$ where $\bar\kappa$ pools cosine $(\boldsymbol{\rho}\cdot\mathbf{S})/(\|\boldsymbol{\rho}\|\|\mathbf{S}\|)$ with per-bin pools $\tilde S_k = G_k * \rho_k$ (**unnormalized** kernel sums), and $\bar z$ pools normalized $\sum_k u_k$. A **fixed** scalar $\eta_z$ (`SEED.ETA_Z`, buffer) is kept for diagnostics only. Diagnostic $\kappa$ is **not** multiplied into $\rho$; it feeds the renderer MLP as $\bar\kappa_{\mathrm{col}}$.
...
Take $\mu_k(c) = \rho_k^{\mathrm{raw}}(c)$ as the nonnegative **drive** for the GABA recurrence in §4 (no sigmoid / logit layer on this grid).
...
Detach $\boldsymbol{\mu}$ once as $\boldsymbol{\mu}^{\mathrm{seed}}$ for the fixed $\beta_{\mathrm{seed}}$ branch. `rho_k_initial` in `cells` stores raw $\boldsymbol{\mu}$ (pre–GABA NR).

**Recurrence** $t = 0,\ldots,T-1$ (`L1.COL_PASSES`). Border cells zero $u$ and $\rho$ after each map.

$$
u_k^{(t)}(c) = \max\Bigl(0,\;
\beta_{\mathrm{seed}}\,\mu_k^{\mathrm{seed}}(c)
...
**Diagnostics.** `kappa_pass0_cell` / `kappa_col_cell` store $\kappa^{(t)}$ after the first / last pass’s $(\boldsymbol{\rho},\mathbf{S})$ pair — first pass uses $\boldsymbol{\rho}^{(0)}=\boldsymbol{\mu}$ (**pre** NR cosine), last pass uses final $\boldsymbol{\rho}$ (**post** NR). Same quantity fed to the η-MLP (before pooling). Not multiplied into $\rho$ in the recurrence.
...
- **Each step** (`prepare_batch` → `run_l1_live_cells` → `HarmonicContourE2E.forward_batch`): L1 runs **live** with `cells_format="torch"` so gradients reach `HypercolumnSeed` ($\tilde\eta_0$, GABA η-MLP, $\tilde\beta_{\cdot}$, $\tilde\alpha_{d,t}$; $\eta_z$ is fixed). `RhoSeedModule` then reads dominant $\rho$; the renderer consumes the same cached L0 pixel tensors (no interleaved L0 recompute inside GABA).
...
| **Total (E2E)** | **170** | `HarmonicContourE2E` |
...
This document matches **raw-space β** GABA on **raw** $\boldsymbol{\mu}$ with **no** seed NR, **one** divisive NR per pass using **spatial** $\eta=\eta_0\cdot\sigma(\mathrm{MLP}(\bar\kappa,\bar z))$, fixed diagnostic $\eta_z$ buffer, **unnormalized** collinear kernel sums $\tilde S_k=G_k*\rho_k$, **14-dimensional** readout, and removal of the **regional η-MLP** from the training and default inference paths. If `HCI_SYSTEM_EQUATIONS.md` exists elsewhere in the project tree, treat it as design narrative unless it explicitly matches this file’s section numbering.
